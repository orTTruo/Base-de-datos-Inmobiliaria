...
class SistemaInmobiliarioApp(ctk.CTk):
    ...
    def crear_paneles_arquitectura(self):
        ...
        # ---------------------------------------------------------------------
        # NUEVO PANEL: DASHBOARD CRM ESTADÍSTICAS
        # ---------------------------------------------------------------------
        ...
        # Contenedor interno para organizar las tarjetas de datos
        self.f_tarjetas = ctk.CTkFrame(f_bloque_crm, fg_color="transparent")
        self.f_tarjetas.pack(fill="both", expand=True, padx=25, pady=10)
        
        # Inicializamos las variables de etiquetas de texto para actualización dinámica
        self.lbl_crm_inmuebles = ctk.CTkLabel(self.f_tarjetas, text="", font=("Arial", 14), anchor="w")
        self.lbl_crm_venta = ctk.CTkLabel(self.f_tarjetas, text="", font=("Arial", 14), anchor="w")
        self.lbl_crm_renta = ctk.CTkLabel(self.f_tarjetas, text="", font=("Arial", 14), anchor="w")
        self.lbl_crm_clientes = ctk.CTkLabel(self.f_tarjetas, text="", font=("Arial", 14), anchor="w")
        self.lbl_crm_asesores = ctk.CTkLabel(self.f_tarjetas, text="", font=("Arial", 14), anchor="w")
        
        # Empaquetado ordenado verticalmente respetando los espaciados originales
        self.lbl_crm_inmuebles.pack(fill="x", pady=8, padx=10)
        self.lbl_crm_venta.pack(fill="x", pady=8, padx=10)
        self.lbl_crm_renta.pack(fill="x", pady=8, padx=10)
        self.lbl_crm_clientes.pack(fill="x", pady=8, padx=10)
        self.lbl_crm_asesores.pack(fill="x", pady=8, padx=10)
...
# =====================================================================
    ...
    # =====================================================================
    # LOGICA CRM: CONSULTA DINÁMICA DE MÉTRICAS EN EL ORDEN SOLICITADO
    # =====================================================================
    def actualizar_metricas_crm(self):
        with db.conectar_bd() as conn:
            cursor = conn.cursor()
            
            # 1. Número total de inmuebles activos (que no están en papelera)
            cursor.execute("SELECT COUNT(*) FROM inmuebles WHERE eliminado != 1")
            total_inmuebles = cursor.fetchone()[0]
            
            # 2. Cuántos inmuebles están bajo régimen de VENTA
            cursor.execute("SELECT COUNT(*) FROM inmuebles WHERE tipo_operacion = 'VENTA' AND eliminado != 1")
            en_venta = cursor.fetchone()[0]
            
            # 3. Cuántos inmuebles están bajo régimen de RENTA
            cursor.execute("SELECT COUNT(*) FROM inmuebles WHERE tipo_operacion = 'RENTA' AND eliminado != 1")
            en_renta = cursor.fetchone()[0]
            
            # 4. Número total de clientes / prospectos registrados
            cursor.execute("SELECT COUNT(*) FROM clientes")
            total_clientes = cursor.fetchone()[0]
            
            # 5. Número total de asesores activos en el sistema
            cursor.execute("SELECT COUNT(*) FROM asesores WHERE activo = 1")
            total_asesores = cursor.fetchone()[0]
            
        # Actualización de textos manteniendo alineaciones limpias
        self.lbl_crm_inmuebles.configure(text=f"🏢 Número total de inmuebles: {total_inmuebles}")
        self.lbl_crm_venta.configure(text=f"   🏷️ Inmuebles en VENTA: {en_venta}")
        self.lbl_crm_renta.configure(text=f"   🔑 Inmuebles en RENTA: {en_renta}")
        self.lbl_crm_clientes.configure(text=f"👥 Número total de clientes: {total_clientes}")
        self.lbl_crm_asesores.configure(text=f"💼 Número de asesores activos: {total_asesores}")
